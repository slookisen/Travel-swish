...
function HomePage({ lang, onStart, onBack }) {
  ...
  function handleStart() {
    ...
    if (apiKey) {
      try { window.localStorage?.setItem('apiKey', apiKey); } catch { }
    }
        ...
        <div style={{ marginBottom: '2rem', background: T.card, border: `1px solid ${T.border}`, borderRadius: '1rem', padding: '1.5rem' }}>
          ...
          {savedKey && !apiKey && (
            <div style={{ marginBottom: '1rem' }}>
              ...
              <button onClick={() => setApiKey('')} style={{ fontSize: '0.85rem', background: T.bg, border: `1px solid ${T.border}`, color: T.txt, padding: '0.5rem 1rem', borderRadius: '0.5rem', cursor: 'pointer' }}>
                {S.home.changeKey}
              </button>
            </div>
          )}
