...
// Mode: restaurants (separate swipe deck)
...
function keysFor(type) {
  // Separate memories per mode so restaurant swipes don't overwrite experience swipes.
  const suffix = type === 'restaurants' ? '_restaurants' : '_experiences';
  return {
...
function loadMemory(type) {
  const K = keysFor(type);
  try {
    const swipes = JSON.parse(localStorage.getItem(K.swipes) || '{}');
    const totalSwipes = parseInt(localStorage.getItem(K.totalSwipes) || '0', 10);
    const seen = JSON.parse(localStorage.getItem(K.seen) || '[]');
    return { swipes, totalSwipes, seen };
  } catch { return { swipes: {}, totalSwipes: 0, seen: [] }; }
}

function saveSwipes(type, swipes, totalSwipes) {
  const K = keysFor(type);
  try {
...
function saveSeen(type, seen) {
  const K = keysFor(type);
  try { localStorage.setItem(K.seen, JSON.stringify(seen)); } catch {}
}

function clearMemory(type) {
  const K = keysFor(type);
  try {
...
function buildPrompt(mode, dest, profileText, lang, excludeNames) {
  ...
  if (lang === 'no') {
    return isRestaurants
      ? `Søk etter restauranter i ${dest} for denne profilen: ${profileText}.${excludeStr}\nReturner en JSON-array med 8-10 restauranter sortert etter match (høyest først). Hvert objekt: {"name":"...","why":"2-3 setninger om hvorfor denne restauranten passer brukerens preferanser","quote":"en kort anmeldelse/sitat fra en ekte anmelder hvis mulig, ellers tom streng","cat":"kjøkken/tema","url":"booking/Google Maps/restaurant-url","price":"ca prisnivå","duration":"(valgfritt)","match":0-100,"lat":0.0,"lng":0.0}. Prioriter steder som er ekte, åpne, og i rimelig avstand. KUN JSON-array, ingen annen tekst.`
  ...
  return isRestaurants
    ? `Search for restaurants in ${dest} for this profile: ${profileText}.${excludeStr}\nReturn a JSON array of 8-10 restaurants sorted by match (highest first). Each object: {"name":"...","why":"2-3 sentences on why this fits the user's preferences","quote":"a short real reviewer quote if available, otherwise empty string","cat":"cuisine/theme","url":"restaurant/Google Maps/reservation URL","price":"price level","duration":"(optional)","match":0-100,"lat":0.0,"lng":0.0}. Prioritize real places near the destination. ONLY JSON array, no other text.`
...
function HomePage({ lang, onStart, onBack, mode, onMode }) {
  const S = STRINGS[lang];
  ...
  function handleStart() {
    ...
    onStart({ destination: dest.trim(), apiKey: key.trim(), mode });
  }
      ...
      <div style={{ flex: 1, maxWidth: '600px', margin: '0 auto', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <div style={{ marginTop: '1.25rem', display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button
              onClick={() => onMode('experiences')}
              style={{
                padding: '0.55rem 1.1rem',
                borderRadius: '2rem',
                cursor: 'pointer',
                border: `1px solid ${T.border}`,
                background: mode === 'experiences' ? `linear-gradient(135deg, ${T.gold}, ${T.teal})` : T.card,
                color: mode === 'experiences' ? T.bg : T.txt,
                fontWeight: 'bold',
              }}
            >
            ...
            <button
              onClick={() => onMode('restaurants')}
              style={{
                padding: '0.55rem 1.1rem',
                borderRadius: '2rem',
                cursor: 'pointer',
                border: `1px solid ${T.border}`,
                background: mode === 'restaurants' ? `linear-gradient(135deg, ${T.gold}, ${T.teal})` : T.card,
                color: mode === 'restaurants' ? T.bg : T.txt,
                fontWeight: 'bold',
              }}
            >
...
function SwipePage({ lang, destination, apiKey, onResults, onBack, mode }) {
  const S = STRINGS[lang];
  const mem = useRef(loadMemory(mode)).current;
  ...
  const cards = (mode === 'restaurants' ? RESTAURANT_CARDS : PREFERENCE_CARDS)[lang];
  ...
  function handleCardSwipe(val) {
    ...
    saveSwipes(mode, newSwipes, newTotal);
...
function TravelSwish() {
  ...
  const [apiKey, setApiKey] = useState('');
  const [mode, setMode] = useState('experiences'); // experiences | restaurants
  ...
  async function handleFindExperiences(prof, overrideMode) {
    ...
    setError('');
    try {
      const mem = loadMemory(activeMode);
      const cards = (activeMode === 'restaurants' ? RESTAURANT_CARDS : PREFERENCE_CARDS)[lang];
      const profile = prof || calcProfile(mem.swipes, cards);
      const profileText = describeProfile(profile, lang);
      const prompt = buildPrompt(activeMode, destination, profileText, lang, seenNames.current);
      ...
      saveSeen(activeMode, seenNames.current);
  ...
  async function handleLoadMore() {
    ...
    try {
      ...
      const prompt = buildPrompt(mode, destination, profileText, lang, allSeen);
      const result = await askClaude(prompt, apiKey);
  ...
  const totalSwipes = parseInt(typeof window !== 'undefined' ? (localStorage.getItem(keysFor(mode).totalSwipes) || '0') : '0', 10);
  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', background: T.bg, fontFamily: 'DM Sans, sans-serif', color: T.txt, overflow: 'hidden' }}>
      ...
      {page === 'home' && <HomePage lang={lang} mode={mode} onMode={setMode} onStart={handleSwipeStart} onBack={() => setPage('landing')} />}
      {page === 'swipe' && <SwipePage lang={lang} mode={mode} destination={destination} apiKey={apiKey} onResults={handleFindExperiences} onBack={() => setPage('home')} />}
