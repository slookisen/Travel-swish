...
_PARTY_TERMS = {
    "solo": "for solo travelers",
    "couple": "for couples",
    "friends": "for friends",
    "family": "for families",
}
_AGE_TERMS = {
    "18-29": "for ages 18 to 29",
    "30-49": "for ages 30 to 49",
    "50-64": "for ages 50 to 64",
    "65+": "for ages 65 plus",
}
_BUDGET_TERMS = {
    ...
    *,
    ...
    if kind == "tours":
        for mapping, key in (
            (_PARTY_TERMS, "party"),
            (_AGE_TERMS, "age_band"),
            (_BUDGET_TERMS, "budget"),
            (_DURATION_TERMS, "duration"),
        ):
    ...
    base = {
        "hotels": "hotel",
        "tours": "organized multi day group tour",
        "custom": query_text or "things to do",
    }[kind]
    ...
    if suffixes:
        candidates.append((f"{base} {destination} {' '.join(suffixes)}", "trip-context", 1.25))
    else:
        candidates.append((f"{base} {destination}", "search-intent", 1.15))
