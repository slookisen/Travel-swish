...
The queue intentionally never writes provider results to disk. Entries are random,
single-use tokens and expire quickly, which keeps the storage transient and scoped
to the active app flow.
"""
...
def mark_failed(token: str, error: str = "prefetch_failed") -> None:
    with _LOCK:
        entry = _ENTRIES.get(token)
        if not entry:
            return
        entry.error = str(error or "prefetch_failed")[:120]
        entry.status = "failed"
        entry.ready.set()
...
def take(token: str, signature: str, *, wait_s: float = 0.0) -> tuple[str, dict[str, Any] | None]:
    """Consume a ready entry once, optionally waiting briefly for completion."""
